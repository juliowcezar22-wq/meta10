...
export function AtividadesPdfClient({ initialData }: { initialData: Material[] }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (result.success) {
      toast('Atividade em PDF salva com sucesso!', 'success')
      closeModal()
      router.refresh()
    } else {
      const errors = result.errors as any
      toast(errors?._form?.[0] || 'Erro ao salvar atividade', 'error')
    }
  }
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(false)
    setDeleteId(null)
    toast('Atividade em PDF excluída com sucesso!', 'success')
    router.refresh()
  }
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto w-full">
      ...
      <div className="bg-white p-6 rounded-2xl border border-surface-200">
        {initialData.length === 0 ? (
          <div className="text-center py-12 bg-surface-50 rounded-xl border border-surface-100">
            <p className="text-surface-500">Nenhuma atividade em PDF cadastrada ainda.</p>
          </div>
        ) : (
          <DataTable 
      ...
      <ConfirmDialog 
        ...
        description={`Tem certeza que deseja excluir a atividade "${deletingMaterial?.title}"? Esta ação não pode ser desfeita.`}
        confirmText="Excluir"
          ...
          <div className="bg-white rounded-2xl w-full max-w-md relative z-10 shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-6 border-b border-surface-100 shrink-0">
              <h2 className="text-xl font-bold text-surface-900">
                {editingMaterial ? 'Editar Atividade' : 'Nova Atividade'}
              </h2>
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-sm font-medium text-surface-700 mb-1">Link do PDF</label>
                <input 
                  type="url" 

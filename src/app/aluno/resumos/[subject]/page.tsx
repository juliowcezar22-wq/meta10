...
import { ArrowLeft, BookOpen, Lock, Download } from 'lucide-react'
import { SUBJECT_LABELS } from '@/lib/constants'

export const dynamic = 'force-dynamic'

export default async function ResumosSubjectPage({ params }: { params: { subject: string } }) {
  await requireAuth()
  
  const subjectName = SUBJECT_LABELS[params.subject] || params.subject
  const materials = await getMaterialsByTypeAndSubject('resumo', params.subject)
  const hasSub = await hasActiveSubscription()

  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto w-full">
      <div className="flex items-center gap-4 mb-8">
        <Link href="/aluno/resumos" className="p-2 hover:bg-surface-100 rounded-lg transition-colors text-surface-500">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-bold text-surface-900">Resumos de <span className="capitalize">{subjectName}</span></h1>
          <p className="text-surface-500">Baixe resumos em PDF para estudar.</p>
        </div>
      </div>
      ...
      {materials.length === 0 && (
        <div className="text-center py-16 bg-white rounded-2xl border border-surface-200 mt-6">
          ...
          <h3 className="text-lg font-bold text-surface-900 mb-1">Nenhum resumo</h3>
          <p className="text-surface-500">Ainda não há resumos para esta disciplina.</p>
        </div>
      )}
    </div>
  )

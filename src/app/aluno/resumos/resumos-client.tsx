'use client'

import { useState } from 'react'
import Link from 'next/link'
import { ArrowLeft, BookOpen, Lock, Eye } from 'lucide-react'
import { MOCK_SUMMARIES } from '@/lib/mock-data'

export default function ResumosClient({ hasSubscription }: { hasSubscription: boolean }) {
  const [selectedDiscipline, setSelectedDiscipline] = useState('Todas')
  const disciplines = ['Todas', ...Array.from(new Set(MOCK_SUMMARIES.map((s) => s.discipline)))]

  const filtered = MOCK_SUMMARIES.filter((s) => selectedDiscipline === 'Todas' || s.discipline === selectedDiscipline)

  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center gap-4 mb-8">
          <Link href="/aluno/dashboard" className="flex items-center gap-2 text-gray-600 hover:text-primary">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Resumos</h1>
            <p className="text-gray-600">Resumos objetivos das matérias mais cobradas.</p>
          </div>
        </div>

        <div className="flex gap-4 mb-6">
          <select
            value={selectedDiscipline}
            onChange={(e) => setSelectedDiscipline(e.target.value)}
            className="px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none bg-white"
          >
            {disciplines.map((d) => (
              <option key={d} value={d}>{d}</option>
            ))}
          </select>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map((summary) => (
            <div key={summary.id} className="card card-hover p-6 relative overflow-hidden">
              {summary.premium && !hasSubscription && (
                <div className="absolute inset-0 bg-gray-50/80 backdrop-blur-sm flex items-center justify-center z-10">
                  <div className="text-center">
                    <Lock className="w-8 h-8 text-gray-400 mx-auto mb-2" />
                    <p className="text-sm text-gray-500 font-medium">Disponível no plano pago</p>
                    <Link href="/planos" className="btn-primary text-sm mt-2">Assinar Plano</Link>
                  </div>
                </div>
              )}
              <div className="w-12 h-12 bg-red-50 rounded-xl flex items-center justify-center mb-4">
                <BookOpen className="w-6 h-6 text-red" />
              </div>
              <span className="text-xs font-medium text-cyan bg-cyan-50 px-2 py-0.5 rounded-full">{summary.discipline}</span>
              <h3 className="font-semibold text-gray-900 mt-2 mb-1">{summary.title}</h3>
              <p className="text-sm text-gray-500 mb-4">{summary.pages} páginas</p>
              {(!summary.premium || hasSubscription) && (
                <button className="btn-primary w-full text-sm">
                  <Eye className="w-4 h-4" />
                  Visualizar
                </button>
              )}
            </div>
          ))}
        </div>
      </div>
    </div>
  )
}

...
**o código novo quebra se for ao ar antes das migrations** (detalhes na
seção "Se a ordem inverter" no final).
...
- [ ] 8. Aplicar **0017** e rodar a verificação (trava removida + 3 funções + estatísticas retornando).
- [ ] 9. **NÃO** executar 0010, 0012 e 0014 (aguardam decisões — ver runbook).
...
- [ ] 11. `NEXT_PUBLIC_HOTMART_MENSAL` e `NEXT_PUBLIC_HOTMART_ANUAL` — **ATENÇÃO:** sem elas, os botões "Assinar Agora" caem em `pay.hotmart.com/placeholder-…` (link quebrado à venda!). As antigas `…_SEMESTRAL`, `…_META10` e `…_GRATUITO` não são mais usadas e podem ser removidas.
...
- [ ] 18. Conta Gratuito: responder 10 questões distintas → a 11ª mostra o bloqueio elegante com CTA para /planos; refazer as 10 continua liberado.
- [ ] 19. A mesma conta com Mensal atribuído → 11ª questão funciona, sem banner de limite.
